...
(function($) {
  ...
  SimpleColorPicker.prototype = {
    ...
    hidePicker: function(event) {
      var picking = this.$picker.is(":visible");
      this.$picker.hide(this.options.delay);
      // click to picker icon, don't trigger blur
      if (event && event.target != this.$icon[0]) {
        // console.log("Picker blur, by clicking on", event.target);
        this.$select.prop("blurBy", event.target);
        this.$select.prop("picking", picking);
        this.$select.blur();
      }
    },
