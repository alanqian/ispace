...
class ImportProductTest < ImportSheetTest
  fixtures :categories  # verify category

  setup do
    @import_sheet = ImportProduct.new(store_id: 1, user_id: 1)
    @ok = ["product.xls"]
    @bad = ['bad_upload.file', "missing_field.xlsx", "bad_sheet_name.xls"]
  end
  ...
  test "b. import xls file" do
    @ok.each do |filename|
      ...
      assert count[:product] > 0 && count[:product] == rows, "import product count=rows"
