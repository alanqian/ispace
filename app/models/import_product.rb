class ImportProduct < ImportSheet
  ...
  # check sheet name as category name or code for non-empty sheets
  ...
  def valid_category?(name_or_code)
    Category.exists?(['code = ? OR name = ?', name_or_code, name_or_code])
  end

  def find_category_code(name_or_code)
    ar = Category.select([:code]).where(['code = ? OR name = ?', name_or_code, name_or_code]).first
    if ar
      return ar.code
    else
      return nil
  ...
  def import_demo(params, row_index)
    ...
    [:brand, :supplier, :manufacturer].each do |k|
      ...
      if params[k]
        ...
        if ar
          ...
          prm[:import_id] = self.id
          params[k][:id] = klass.create(prm).id
          @count[k] += 1
        end
      end
    end
    ...
    if params[:product]
      if params[:brand][:id] && params[:supplier][:id]
        if params[:manufacturer]
          mfr_id = params[:manufacturer][:id]
        else
          mfr_id = -1
        end
        prm = params[:product].merge(m)
        ar = Product.where({ code: prm[:code]}).first
        if ar
          product_id = ar.id
          logger.debug "product import skiped, id:#{product_id}, import_id:#{self.id}"
        else
          prm.merge!({
            import_id: self.id,
          ...
          product_id = Product.create(prm).id
          @count[:product] += 1
          logger.debug "product imported, id:#{product_id}, import_id:#{self.id}"
        end
      else
        logger.warn "product w/o brand or supplier, product:#{params[:product]} brand:#{params[:brand]} supplier:#{params[:supplier]}"
        return false
      end
    end
    true
